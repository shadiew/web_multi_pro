...
class FW_Option_Type_FwMultiInline extends FW_Option_Type {
	...
	protected function _render( $id, $option, $data ) {
		return fw_render_view( DIGIQOLE_COMPONENTS . '/option-types/fw-multi-inline/view.php', array(
			'id'	 => $id,
			'option' => $option,
			'data'	 => $data
		) );
		...
		return array(
			...
			'fw_multi_options'	 => array(
				'firstoption' => array(
					...
					'choices'	 => array(
						'select1'	 => esc_html__( 'select1', 'digiqole' ),
						'select2'	 => esc_html__( 'select2', 'digiqole' ),
						'select3'	 => esc_html__( 'select3', 'digiqole' )
					),
				)
			),
			'groupname'			 => ''
