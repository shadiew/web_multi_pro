...
*/
    class CMC_Settings
    {
        ...
        {
            $this->Base_File = CMC_PRO_FILE;
            $this->PLUGIN_NAME = CMC_ApiConf::PLUGIN_NAME;
            $this->PREFIX = CMC_ApiConf::PLUGIN_PREFIX;
            $this->AUTH_PAGE = CMC_ApiConf::PLUGIN_AUTH_PAGE;
            $this->PLUGIN_URL = CMC_ApiConf::PLUGIN_URL;
            $this->PLUGIN_VER = CMC_ApiConf::PLUGIN_VERSION;
            $this->plugin_purchase_url = 'https://1.envato.market/cmc';
            $this->plugin_documentation_url = 'https://docs.coolplugins.net/doc/coin-market-cap-prices/';
  ...
  /*
            $this->settings_api->add_section( $this->PREFIX . '_license_welcome', __('Welcome','cmb2') );
            $this->settings_api->add_field( $this->PREFIX . '_license_welcome', array(
                array(
...
*/
            $this->settings_api->add_section( $this->PREFIX . '_license_registration', __('CMC Registration', 'cmb2') );
        ...
        */
        ...
        function welcome_tab(){
            ...
            <h4>Cryptocurrency Widgets Pro</h4>
            <a href="https://1.envato.market/cryptocurrency" target="_blank" class="cwp-ad-banner"><img style="max-width:100%;height:auto;border:2px solid #d8d8d8;padding:4px;" src="https://res.cloudinary.com/pinkborder/image/upload/v1565162802/CoinMarketCap-Plugin/widgets-pro-ad.png" /></a><br/>
            <h4>Coins Marketcap</h4>
            <a href="https://1.envato.market/cmc" target="_blank" class="cmc-ad-banner"><img style="max-width:100%;height:auto;border:2px solid #d8d8d8;padding:4px;" src="https://res.cloudinary.com/pinkborder/image/upload/v1565162803/CoinMarketCap-Plugin/coins-marketcap-ad.png" /></a>
            </p>';
        ...
        public function ValidatePurchase()
        {
           ...
           if( isset( $_GET['settings-updated'] ) || !empty($purchase) )
           {
                if( !empty( $purchase ) ){
                    ...
                    $verified = CoinsMarketCapBase::CheckWPPlugin($purchase,$admin_email,$this->licenseMessage,$this->Response,$this->Base_File);
        ...
        */
        function uninstall_license(){
            ...
            if( wp_verify_nonce( $_REQUEST['_password'], 'purchase-verify'  ) == true ){
               $response = CoinsMarketCapBase::RemoveLicenseKey($this->Base_File,$message);
        ...
        */
        function load_settings_scripts(){
            ...
            if( isset($_GET['page']) && $_GET['page']== 'cool-crypto-registration' ){
                wp_enqueue_style( $this->PREFIX.'-settings-style', $this->PLUGIN_URL . 'assets/css/admin/api-auth-settings.min.css',null,$this->PLUGIN_VER);
                wp_enqueue_script( $this->PREFIX.'-settings-script', $this->PLUGIN_URL . 'assets/js/admin/api-auth-settings.min.js', array('jquery'), $this->PLUGIN_VER);
                wp_localize_script(
