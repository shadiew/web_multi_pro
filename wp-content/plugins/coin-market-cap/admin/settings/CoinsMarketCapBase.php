...
namespace CoinMarketCapREG;
	...
	class CoinsMarketCapBase {
    	public $key = "BB888C000D21F2CB";
    	private $product_id = "1";
    	private $product_base = "wp-cmc";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_wp-cmc_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=wp-cmc_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
					}
			...
			//if(empty($response->code)) {
				//if ( ! empty( $response->status ) ) {
					//if ( ! empty( $response->data ) ) {
            			...
            			$licenseObj->request_duration = 50000;
            			$licenseObj->expire_date = strtotime("+ 365 day");
            			$licenseObj->support_end = 'Unlimited';
            			$licenseObj->license_title = "PRO";
            			$response->msg = "Activated with key: B25D2E81-E1F90D83-AEA9DA8B-8F7686E0";
