...
use crawlomatic_andreskrey\Readability\Nodes\DOM\DOMDocument;
use crawlomatic_andreskrey\Readability\Nodes\DOM\DOMElement;
use crawlomatic_andreskrey\Readability\Nodes\DOM\DOMNode;
use crawlomatic_andreskrey\Readability\Nodes\DOM\DOMText;
use crawlomatic_andreskrey\Readability\Nodes\NodeUtility;
use Psr\Log\LoggerInterface;

/**
 * Class Readability.
 */
if(!class_exists('ReadabilityCrawlomatic'))
{
class ReadabilityCrawlomatic
{
    /**
    ...
    public function parse($html)
    {
        ...
        while (true) {
            ...
            {
                $length = strlen(preg_replace(NodeUtility::$regexps['onlyWhitespace'], '', $result->textContent));
            }
            ...
            if ($elementProperty) {
                if (preg_match($propertyPattern, $elementProperty, $matches)) {
                    for ($i = count($matches) - 1; $i >= 0; $i--) {
                        ...
                        {
                            $name = preg_replace('/\s/', '', strtolower($matches[$i]));
                        }
                        // multiple authors
    ...
    private function isValidByline($text)
    {
        if (gettype($text) == 'string') {
            ...
            {
                return (strlen($byline) > 0) && (strlen($byline) < 100);
            }
        }
    ...
    private function rateNodes($nodes)
    {
        ...
        foreach ($nodes as $node) {
            ...
            {
                if (strlen($node->getTextContent(true)) < 25) {
                    continue;
                }
            }
            ...
            {
                $contentScore += min(floor(strlen($node->getTextContent(true)) / 100), 3);
            }
    ...
    public function prepArticle(DOMDocument $article)
    {
        ...
        if ($h2->length === 1) {
            ...
            {
                $lengthSimilarRate = (strlen($h2->item(0)->textContent) - strlen($this->getTitle())) / max(strlen($this->getTitle()), 1);
            }
    ...
    public function _cleanConditionally(DOMDocument $article, $tag)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            if (substr_count($node->getTextContent(), ',') < 10) {
                ...
                {
                    $contentLength = strlen($node->getTextContent(true));
                }
